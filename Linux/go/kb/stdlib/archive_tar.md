...
Write files into a tar archive with a Writer (WriteHeader then Write per entry), then read them back by iterating tr.Next until io.EOF and io.Copy-ing each entry. Keywords: tar NewWriter NewReader WriteHeader Write Close Next Header io.Copy create archive extract untar bundle files tarball iterate entries.
